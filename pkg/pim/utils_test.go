...
func TestParseDateTime(t *testing.T) {
	...
	dateTZ := time.Date(2024, 12, 31, 0, 0, 0, 0, time.Local).Format("-07:00")
	...
	assert.Equal(t, fmt.Sprintf("2024-12-31T00:00:00%s", dateTZ), dateOnly, errMsg)
	assert.Equal(t, fmt.Sprintf("%sT13:37:00%s", currentDate, currentTZ), timeOnly, errMsg)
	assert.Equal(t, fmt.Sprintf("2024-12-31T13:37:00%s", dateTZ), dateTime, errMsg)
}
